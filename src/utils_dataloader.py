...
def parse_fasta_file_greengenes2(file: str, min_length: int = 0, max_length: int = 10_000_000) -> pd.DataFrame :
    """
...
def parse_fasta_file_gtdb_gzip(file: str, domain: str, min_seq_length: int = None, max_seq_length: int = 10_000_000) -> pd.DataFrame :
    """
        Read GTDB gzip fasta file
    """

    rows_list = []
    columns = ['ID','Taxon','Kingdom','Phylum','Class','Order','Family','Genus','Species','SeqLen','Seq']
            ...
            if str(rec.description).split()[1].startswith(domain):
                ...
                if len(taxon_split) == 7:
                    ...
                    dict1 = dict( (col, val) for (col, val) in 
                                  zip(columns, [id, taxon, kingdom, phylum, _class, order, family, genus, species, seq_len, seq]))
