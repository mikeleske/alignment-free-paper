...
def preprocess_full16s(
    DATA_FILE: Path, TAXONOMY_FILE: Path, MIN_LENGTH: int, MAX_LENGTH: int
) -> pd.DataFrame:
    #
    # Read the sequences and filter on relevant length
    #
    df = parse_fasta_file_greengenes2(
        file=DATA_FILE, min_length=MIN_LENGTH, max_length=MAX_LENGTH
    )
...
def main():
    DATA_FILE = Path(
        "../data/raw/5b42d9b6-2f24-4f01-b989-9b4dafca7d5e/data/dna-sequences.fasta"
    )
    TAXONOMY_FILE = Path(
        "../data/raw/b7c3e691-ea51-4547-94dd-f79f49e41a36/data/taxonomy.tsv"
    )
    ...
    df.to_csv(
        Path("../data/gg2.2024.09.backbone.full-length.csv.gz"),
        sep="\t",
        index=False,
        compression="gzip",
    )
    ...
    df.to_csv(
        Path("../data/gg2.2024.09.backbone.full-length.V3V4.csv.gz"),
        sep="\t",
        index=False,
        compression="gzip",
    )
