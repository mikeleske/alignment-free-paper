...
def split_and_explode_to_columns(
    df: pd.DataFrame, column: str, delimiter: str = "; "
) -> pd.DataFrame:
    """
...
def preprocess_full16s(
    DATA_FILE: Path, MIN_LENGTH: int, MAX_LENGTH: int
) -> pd.DataFrame:
    #
    # Read the sequences and filter on relevant length
    #
    df = parse_fasta_file_gtdb_gzip(file=DATA_FILE, domain="d__Bacteria")
    ...
    # Remove duplicates
    #
    df = df.drop_duplicates(subset=["Taxon", "Seq"], keep="first")

    #
    # Merge dedup data with counts

    df = pd.merge(
        df, df_counts, how="left", left_on=["Taxon", "Seq"], right_on=["Taxon", "Seq"]
    )
...
def preprocess_v3v4(DATA_FILE: Path) -> pd.DataFrame:
    ...
    # Remove duplicates
    #
    df = df.drop_duplicates(subset=["Taxon", region], keep="first")

    #
    # Merge dedup data with counts
    #
    df = pd.merge(
        df, df_counts, how="left", left_on=["Taxon", region], right_on=["Taxon", region]
    )
...
def main():
    ...
    logging.info("Preprocessing and deduplicating - Full16S")
    MIN_LENGTH = 1400
    MAX_LENGTH = 1600
    df = preprocess_full16s(
        DATA_FILE=DATA_FILE, MIN_LENGTH=MIN_LENGTH, MAX_LENGTH=MAX_LENGTH
    )
    print_stats(df)
    logging.info("Saving dataframe as gtdb.r220.full-length.csv.gz")
    df.to_csv(
        Path("../data/gtdb.r220.full-length.csv.gz"),
        sep="\t",
        index=False,
        compression="gzip",
    )
    logging.info("Preprocessing and deduplicating - Full16S - Completed")

    # Preprocess V3V4 data
    logging.info("Preprocessing and deduplicating - V3V4")
    df = preprocess_v3v4(DATA_FILE=DATA_FILE)
    print_stats(df)
    logging.info("Saving dataframe as gtdb.r220.full-length.V3V4.csv.gz")
    df.to_csv(
        Path("../data/gtdb.r220.full-length.V3V4.csv.gz"),
        sep="\t",
        index=False,
        compression="gzip",
    )
    logging.info("Preprocessing and deduplicating - V3V4 - Completed")
